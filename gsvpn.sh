...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# download OpenSSL 1.1.1 to make sure we don't get 'nopass' bug 
			...
			dkpg -i libssl1.1_1.1.1f-1ubuntu2.17_amd64.deb
			dpkg -i libssl-dev_1.1.1f-1ubuntu2.17_amd64.deb
			dpkg -i openssl_1.1.1f-1ubuntu2.17_amd64.deb
			apt-get install -y openvpn iptables ca-certificates curl
		elif [[ $OS == 'centos' ]]; then
...
function newClient() {
	...
	#check for autogeneration params
	AUTOCLIENTTYPE=$1
	AUTOCLIENTNAME=$2
	AUTONETNUM=$3
	if [[ $AUTOCLIENTTYPE ]]; then

	echo "type: $AUTOCLIENTTYPE";
	echo "client: $AUTOCLIENTNAME";
	echo "prod net num: $AUTONETNUM";
	echo "New auto client..."
...
# echo "type: $AUTOCLIENTTYPE";
# echo "client: $AUTOCLIENTNAME";
# echo "net num: $AUTONETNUM";
echo "Starting auto client generation."
newClient "$AUTOCLIENTTYPE" "$AUTOCLIENTNAME" "$AUTONETNUM"
fi
